...
Para nuestro primer ejemplo usaremos la función <<\pynormal{print}>> definida por defecto en Python. Escribe el comando <<\pynormal{print("¡Hola mundo!")}>> en tu sesión de Python y presionamos Enter. El resultado debería ser el siguiente:
...
Para salir de Python y volver a una terminal de comandos normal, debes utilizar la función <<\pynormal{exit}>> también definida por defecto. Específicamente, debes escribir el comando <<\pynormal{exit()}>> y presionar Enter.
...
Una vez instalado, abre Geany desde el menú de aplicaciones. En la ventana del editor, escribe la instrucción \pynormal{print("¡Hola mundo!")} y utiliza la combinación de teclas \bashbold{Ctrl+S} para guardar el archivo. Puedes guardar el archivo con el nombre \norbash{hello_world.py} en tu carpeta de trabajo. Asegúrate de colocar la extensión \norbash{.py} en el nombre de tu archivo al momento de guardarlo. 
...
En esa nueva pestaña, verás que aparecen los caracteres \pynormal{[]:} (encerrado con un círculo rojo en el panel izquierdo de la Figura \ref{fig:jupyter-code-markdown}) y a la derecha aparece una celda vacía donde puedes escribir los comandos de Python. Los caracteres \pynormal{[]:} en Jupyter Notebook son el equivalente a los caracteres \pynormal{>>>} en la terminal. 

En la parte superior del Notebook hay una barra de menú, que a su vez contiene un botón con la palabra <<\norbash{Code}>> (encerrada con un rectángulo rojo en el panel izquierdo de la Figura \ref{fig:jupyter-code-markdown}). Al darle clic se muestra un menú desplegable cuyas opciones son <<\norbash{Code}>> y <<\norbash{Markdown}>>. Este último es un tipo de \emph{lenguaje de marcas} bastante popular para escribir texto con formato. Si seleccionas <<\norbash{Markdown}>>, los símbolos \pynormal{[]:} de la celda desaparecen (como se muestra en el panel derecho de la Figura \ref{fig:jupyter-code-markdown}) y ahora puedes escribir texto, ecuaciones, imágenes y más. Puedes revisar algunas nociones básicas sobre la escritura de texto Markdown en este enlace de \href{https://docs.github.com/en/get-started/writing-on-github/getting-started-with-writing-and-formatting-on-github/basic-writing-and-formatting-syntax}{Github Docs}. 

Cuando vas a escribir código, siempre debes verificar que la celda está en modo <<\norbash{Code}>> o en su defecto, que a la izquierda de la celda aparezcan los símbolos \pynormal{[]:}, de lo contrario, tus comandos no se ejecutarán. Por ejemplo, da clic en la celda vacía y asegúrate que se encuentre en modo <<\norbash{Code}>> y escribe el comando \pynormal{print("¡Hola mundo!")}. Para ejecutar el código de una celda, debes dar click a la celda y usar la combinación de teclas \bashbold{Shift+Enter}. El resultado debería ser algo similar a lo siguiente:
...
Una de las formas elementales en las que Python puede ser utilizado es como una calculadora. En este sentido, la Tabla \ref{tab:aritmetic-operators} muestra las operaciones aritméticas que pueden ejecutarse con Python. Quizá no estés muy familiarizado con las operaciones de división entera (representada con los caracteres \pynormal{//}) y división modular (representada con el carácter \mintinline[style=friendly]{python}{%}), pero las revisaremos con detalle más adelante.
...
En este caso, \pynormal{7 // 3} es igual a $2$ porque la parte fraccionaria ($0.333...$) se descarta. Ahora veamos cómo se comporta este operador con números negativos:
\begin{pyin}
-7 // 3
\end{pyin}
\begin{pyout}
...
En este caso, \pynormal{-7 // 3} es igual a \pynormal{-3} porque el resultado se redondea hacia abajo (es decir, al entero más negativo).
...
Para realizar operaciones matemáticas más avanzadas, necesitaremos utilizar un \emph{módulo} de Pytho llamado \pynormal{math}. Un módulo es un archivo que contiene definiciones y declaraciones de Python, como funciones, variables y clases. Los módulos permiten organizar y reutilizar código.

Para usar el módulo \pynormal{math}, debemos importarlo en nuestro código. La forma más sencilla de hacerlo es usando la palabra clave <<\pynormal{import}>>. Dentro del módulo \pynormal{math} están definidas muchas funciones matemáticas como la raíz cuadrada, logaritmos, funciones trigonométricas y mucho más. Para usarlas, debemos hacerlo con el operador de acceso, que en Python es un punto. La siguiente celda de código muestra cómo importar el módulo \pynormal{math} y cómo acceder a la función raíz cuadrada, definida como \pynormal{sqrt} dentro de dicho módulo:
...
También podemos hacer importaciones específicas de las funciones dentro del módulo. Para esto usamos la palabra <<\pynormal{from}>>. El siguiente ejemplo muestra cómo importar la función \pynormal{sqrt} definida dentro del módulo \pynormal{math}:
\begin{pyin}
...
4.0
\end{pyprint}
En este último caso, no fue necesario escribir \pynormal{math.sqrt} para calcular la raíz cuadrada, porque importamos directamente la función \pynormal{sqrt}. 

Adicionalmente, puedes importar módulos o funciones específicas y renombrarlas a tu gusto usando la palabra <<\pynormal{as}>>. Los siguientes dos ejemplos muestran cómo funciona esta sintaxis.
